...
class SobyRenderer < Java::ProcessingOpengl::PGraphicsOpenGL
...
#        puts "EVAL #{desc}"
  ...
  def mouse_dragged
    if not @is_moving
      tr = PMatrix3D.new
      tr.translate(parent.mouse_x - parent.pmouse_x,
                   parent.mouse_y - parent.pmouse_y)
  ...
  def goto_slide (next_slide)
    ...
    @transition_start_time = parent.millis()
    @is_moving = true
    @current_ratio = 0
  ...
  def update_cam
    ...
    elapsed_time = parent.millis - @transition_start_time
